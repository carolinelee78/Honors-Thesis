...
ggstatsplot::ggscatterstats(datScreened, "PPIR_FearDom","FERT_Score")
ggstatsplot::ggscatterstats(datScreened, "PPIR_SCImp","FERT_Score")
ggstatsplot::ggscatterstats(datScreened, "PPIR_C","FERT_Score")

model.1 = lm(scale(FERT_Score) ~ scale(PPIR_FearDom) + scale(PPIR_SCImp) + scale(PPIR_C), data=datScreened2)
summary(model.1)
...
model.2 = lm (scale(FERT_CorrectEstimate) ~ scale(FERT_Score) + FERT_Score_SQ, data=datScreened2)

model.3 = lm (FERT_CorrectEstimate ~FERT_Score + FERT_Score_SQ + FERT_Score_cubed, data=datScreened2)
...
ggplot(datScreened2, aes(x=FERT_Score, y=FERT_CorrectEstimate)) + 
  geom_point() +
  stat_smooth(se=F, method='lm', formula=y~x, color = "cadetblue1") + 
  stat_smooth(se=F, method='lm', formula=y~poly(x,2), color = "pink") + 
  stat_smooth(se=F, method='lm', formula=y~poly(x,3), color = "aquamarine1") + 
  theme_classic()
...
####################
datScreenedConditions <- datScreened %>% mutate(Cooperative = ifelse(condition == 1, "Cooperative", 
                                                                     ifelse(condition == 2, "Competitive",
                                                                            ifelse(condition == 3, "Cooperative",
                                                                                   "Competitive"))),
                                                Stroop = ifelse(condition == 1 | condition == 2, "Stroop", "Non-Stroop")
)


model.conditionCompare1<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp), data = datScreenedConditions)
model.conditionCompare2<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) + Cooperative, data = datScreenedConditions)
model.conditionCompare3<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) * Cooperative, data = datScreenedConditions)


model.conditionCompare1<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp), data = datScreenedConditions)
model.conditionCompare2<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) + Stroop, data = datScreenedConditions)
model.conditionCompare3<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) * Stroop, data = datScreenedConditions)

model.conditionCompare2<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) + Stroop + Cooperative, data = datScreenedConditions)
model.conditionCompare3<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp) + Stroop * Cooperative, data = datScreenedConditions)
model.conditionCompare4<-lm(IPD_Score ~ PPIR_SCImp * factor(Stroop) * factor(Cooperative) -1, data = datScreenedConditions)

sjPlot::tab_model(model.conditionCompare4 , show.std=T)


model.conditionCompare4<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp)*Stroop, data = datScreenedConditions)

model.conditionCompare4SCI<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp)*Cooperative*Stroop, data = datScreenedConditions)
model.conditionCompareCH<-lm(scale(IPD_Score) ~ scale(PPIR_C)*Cooperative*Stroop, data = datScreenedConditions)

summary(model.conditionCompareCH)

model.conditionCompare<-lm(scale(IPD_Score) ~ scale(PPIR_C)*factor(condition) -1 , data = datScreenedConditions)
model.conditionCompare<-lm(scale(IPD_Score) ~ scale(PPIR_SCImp)*factor(condition) -1 , data = datScreenedConditions)

summary(model.conditionCompare)


sjPlot::tab_model(model.conditionCompare, show.std=T)

ggbetweenstats(
  datScreenedConditions,
  condition,
  IPD_Score,
  plot.type = "boxviolin",
  type = "parametric")

interaction <- aov(IPD_Score ~ Cooperative*Stroop, data = datScreenedConditions)

summary(interaction)

datScreenedConditions %>%
  group_by(Cooperative, Stroop) %>%
  summarise(mean = mean(IPD_Score),
            sd = sd(IPD_Score))


TukeyHSD(interaction, conf.level=.95) 
...
PPICitems<-datScreened %>% select(PPIR_27_r:PPIR_153_r)
PPI_SCI_items<-datScreened %>% select(c(PPIR_33:PPIR_154, PPIR_4:PPIR_149, PPIR_89_r:PPIR_130_r))
PPI_FD_items<-datScreened %>% select(c(PPIR_87_r:PPIR_46, PPIR_47_r:PPIR_148, PPIR_10_r:PPIR_140))

psych::alpha(PPICitems)
psych::alpha(PPI_SCI_items)
psych::alpha(PPI_FD_items)