...
InputData <- read.csv("/Users/carolinelee/Desktop/Honors/Undergrad/4_undergrad_s3.csv")
...
R1_Strategy <- ifelse(as.character(InputData[64, "response"]) == "g", "Cooperate", "Defect")

R1_Outcome <- as.character(InputData[68, "stimulus"])

R1_Outcome <- toupper(str_sub(R1_Outcome, 62, -33))

R1_AffectiveFeedback <- as.numeric(as.character(InputData[68, "response"]))
...
R1_CoopExpect <- as.numeric(as.character(InputData[70, "response"])) / 100

# Round 2 

R2_Strategy <- ifelse(as.character(InputData[71, "response"]) == "g", "Cooperate", "Defect")

R2_Outcome <- as.character(InputData[74, "stimulus"])

R2_Outcome <- toupper(str_sub(R2_Outcome, 62, -33))

R2_AffectiveFeedback <- as.numeric(as.character(InputData[74, "response"]))
...
R2_CoopExpect <- as.numeric(as.character(InputData[76, "response"])) / 100

# Round 3

R3_Strategy <- ifelse(as.character(InputData[77, "response"]) == "g", "Cooperate", "Defect")

R3_Outcome <- as.character(InputData[80, "stimulus"])

R3_Outcome <- toupper(str_sub(R3_Outcome, 62, -33))

R3_AffectiveFeedback <- as.numeric(as.character(InputData[80, "response"]))
...
R3_CoopExpect <- as.numeric(as.character(InputData[82, "response"])) / 100

# Round 4

R4_Strategy <- ifelse(as.character(InputData[83, "response"]) == "g", "Cooperate", "Defect")

R4_Outcome <- as.character(InputData[86, "stimulus"])

R4_Outcome <- toupper(str_sub(R4_Outcome, 62, -33))

R4_AffectiveFeedback <- as.numeric(as.character(InputData[86, "response"]))
...
R4_CoopExpect <- as.numeric(as.character(InputData[88, "response"])) / 100

# Round 5 

R5_Strategy <- ifelse(as.character(InputData[89, "response"]) == "g", "Cooperate", "Defect")

R5_Outcome <- as.character(InputData[92, "stimulus"])

R5_Outcome <- toupper(str_sub(R5_Outcome, 62, -33))

R5_AffectiveFeedback <- as.numeric(as.character(InputData[92, "response"]))
...
R5_CoopExpect <- as.numeric(as.character(InputData[94, "response"])) / 100

# Round 6

R6_Strategy <- ifelse(as.character(InputData[95, "response"]) == "g", "Cooperate", "Defect")

R6_Outcome <- as.character(InputData[98, "stimulus"])

R6_Outcome <- toupper(str_sub(R6_Outcome, 62, -33))

R6_AffectiveFeedback <- as.numeric(as.character(InputData[98, "response"]))
...
R6_CoopExpect <- as.numeric(as.character(InputData[100, "response"])) / 100

# Round 7 

R7_Strategy <- ifelse(as.character(InputData[101, "response"]) == "g", "Cooperate", "Defect")

R7_Outcome <- as.character(InputData[104, "stimulus"])

R7_Outcome <- toupper(str_sub(R7_Outcome, 62, -33))

R7_AffectiveFeedback <- as.numeric(as.character(InputData[104, "response"]))
...
R7_CoopExpect <- as.numeric(as.character(InputData[106, "response"])) / 100

# Round 8 

R8_Strategy <- ifelse(as.character(InputData[107, "response"]) == "g", "Cooperate", "Defect")

R8_Outcome <- as.character(InputData[110, "stimulus"])

R8_Outcome <- toupper(str_sub(R8_Outcome, 62, -33))

R8_AffectiveFeedback <- as.numeric(as.character(InputData[110, "response"]))
...
R8_CoopExpect <- as.numeric(as.character(InputData[112, "response"])) / 100

# Round 9 

R9_Strategy <- ifelse(as.character(InputData[113, "response"]) == "g", "Cooperate", "Defect")

R9_Outcome <- as.character(InputData[116, "stimulus"])

R9_Outcome <- toupper(str_sub(R9_Outcome, 62, -33))

R9_AffectiveFeedback <- as.numeric(as.character(InputData[116, "response"]))
...
R9_CoopExpect <- as.numeric(as.character(InputData[118, "response"])) / 100

# Round 10 

R10_Strategy <- ifelse(as.character(InputData[119, "response"]) == "g", "Cooperate", "Defect")

R10_Outcome <- as.character(InputData[122, "stimulus"])

R10_Outcome <- toupper(str_sub(R10_Outcome, 62, -33))

R10_AffectiveFeedback <- as.numeric(as.character(InputData[122, "response"]))
...
R10_CoopExpect <- as.numeric(as.character(InputData[124, "response"])) / 100

# Round 11 

R11_Strategy <- ifelse(as.character(InputData[125, "response"]) == "g", "Cooperate", "Defect")

R11_Outcome <- as.character(InputData[128, "stimulus"])

R11_Outcome <- toupper(str_sub(R11_Outcome, 62, -33))

R11_AffectiveFeedback <- as.numeric(as.character(InputData[128, "response"]))
...
R11_CoopExpect <- as.numeric(as.character(InputData[130, "response"])) / 100

# Round 12

R12_Strategy <- ifelse(as.character(InputData[131, "response"]) == "g", "Cooperate", "Defect")

R12_Outcome <- as.character(InputData[134, "stimulus"])

R12_Outcome <- toupper(str_sub(R12_Outcome, 62, -33))

R12_AffectiveFeedback <- as.numeric(as.character(InputData[134, "response"]))
...
R12_CoopExpect <- as.numeric(as.character(InputData[136, "response"])) / 100

# Round 13

R13_Strategy <- ifelse(as.character(InputData[137, "response"]) == "g", "Cooperate", "Defect")

R13_Outcome <- as.character(InputData[140, "stimulus"])

R13_Outcome <- toupper(str_sub(R13_Outcome, 62, -33))

R13_AffectiveFeedback <- as.numeric(as.character(InputData[140, "response"]))
...
R13_CoopExpect <- as.numeric(as.character(InputData[142, "response"])) / 100

# Round 14

R14_Strategy <- ifelse(as.character(InputData[143, "response"]) == "g", "Cooperate", "Defect")

R14_Outcome <- as.character(InputData[146, "stimulus"])

R14_Outcome <- toupper(str_sub(R14_Outcome, 62, -33))

R14_AffectiveFeedback <- as.numeric(as.character(InputData[146, "response"]))
...
R14_CoopExpect <- as.numeric(as.character(InputData[148, "response"])) / 100

# Round 15

R15_Strategy <- ifelse(as.character(InputData[149, "response"]) == "g", "Cooperate", "Defect")

R15_Outcome <- as.character(InputData[152, "stimulus"])

R15_Outcome <- toupper(str_sub(R15_Outcome, 62, -33))

R15_AffectiveFeedback <- as.numeric(as.character(InputData[152, "response"]))
...
R15_CoopExpect <- as.numeric(as.character(InputData[154, "response"])) / 100

# Round 16

R16_Strategy <- ifelse(as.character(InputData[155, "response"]) == "g", "Cooperate", "Defect")

R16_Outcome <- as.character(InputData[158, "stimulus"])

R16_Outcome <- toupper(str_sub(R16_Outcome, 62, -33))

R16_AffectiveFeedback <- as.numeric(as.character(InputData[158, "response"]))
...
R16_CoopExpect <- as.numeric(as.character(InputData[160, "response"])) / 100

# Round 17 

R17_Strategy <- ifelse(as.character(InputData[161, "response"]) == "g", "Cooperate", "Defect")

R17_Outcome <- as.character(InputData[164, "stimulus"])

R17_Outcome <- toupper(str_sub(R17_Outcome, 62, -33))

R17_AffectiveFeedback <- as.numeric(as.character(InputData[164, "response"]))
...
R17_CoopExpect <- as.numeric(as.character(InputData[166, "response"])) / 100

# Round 18 

R18_Strategy <- ifelse(as.character(InputData[167, "response"]) == "g", "Cooperate", "Defect")

R18_Outcome <- as.character(InputData[170, "stimulus"])

R18_Outcome <- toupper(str_sub(R18_Outcome, 62, -33))

R18_AffectiveFeedback <- as.numeric(as.character(InputData[170, "response"]))
...
R18_CoopExpect <- as.numeric(as.character(InputData[172, "response"])) / 100

# Round 19

R19_Strategy <- ifelse(as.character(InputData[173, "response"]) == "g", "Cooperate", "Defect")

R19_Outcome <- as.character(InputData[176, "stimulus"])

R19_Outcome <- toupper(str_sub(R19_Outcome, 62, -33))

R19_AffectiveFeedback <- as.numeric(as.character(InputData[176, "response"]))
...
R19_CoopExpect <- as.numeric(as.character(InputData[178, "response"])) / 100

# Round 20 

R20_Strategy <- ifelse(as.character(InputData[179, "response"]) == "g", "Cooperate", "Defect")

R20_Outcome <- as.character(InputData[182, "stimulus"])

R20_Outcome <- toupper(str_sub(R20_Outcome, 62, -33))

R20_AffectiveFeedback <- as.numeric(as.character(InputData[182, "response"]))
...
PPI <- as.character(InputData[184, "response"])
